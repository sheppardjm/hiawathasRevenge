...
- ✓ Complete OpenGraph + Twitter Card meta set (og:site_name, og:locale, og:image:alt/type/width/height, twitter:image:alt, theme-color #1a2e1a) — v1.11
- ✓ Google Event rich-result-eligible JSON-LD (street address, timezone-aware startDate CDT -05:00, free-event offers block, geo coordinates, url) — v1.11
- ✓ Canonical URL present and correct on every page — v1.11
- ✓ Shield-badge favicon across all targets — favicon.svg, 180x180 apple-touch-icon.png, 32x32 favicon.ico — pipeline-generated on every build — v1.11
- ✓ Branded 1200x630 og-card.jpg share image (shield badge + tagline + "June 6, 2026" over dimmed hero) with cache-busting filename — v1.11
- ✓ robots.txt + auto-generated sitemap (@astrojs/sitemap) excluding /admin and /api/, referencing sitemap-index.xml — v1.11
...
- Outstanding human-needed verification gaps and the Phase 53 UAT (3 pending scenarios) acknowledged at v1.11 close (see STATE.md → Deferred Items)
...
Shipped v1.0 through v1.11 with 6,211 LOC across Astro/TypeScript/JavaScript/CSS. All 8 gravel sectors defined (520, NF2266, Bass Lake Rd, NF2217-2218, ND2225, Little Indian, Doe Lake, Ridge Rd).
Tech stack: Astro 6, Tailwind 4, Vite 7, Leaflet, Chart.js, PhotoSwipe, leaflet.markercluster, chartjs-plugin-annotation, sharp, gpxparser, @astrojs/sitemap, to-ico.
...
v1.11 completed the SEO & social-sharing layer: full OG/Twitter meta set + Google-eligible Event JSON-LD in BaseLayout.astro, a shield-badge favicon set (SVG/apple-touch/ICO) and branded og-card.jpg both generated by the build pipeline, and robots.txt + @astrojs/sitemap crawlability.
...
| og:locale underscore en_US (not en-US) | Facebook/parsers silently ignore the hyphen BCP47 variant | ✓ Good — v1.11 |
| theme-color literal hex #1a2e1a | CSS custom properties are not evaluated inside content="" attributes | ✓ Good — v1.11 |
| Event startDate offset -05:00 (CDT) | Michigan UP observes CDT in June, not EST/EDT | ✓ Good — v1.11 |
| offers.price string "0", isAccessibleForFree boolean true | Schema.org type spec — wrong types fail rich-result eligibility | ✓ Good — v1.11 |
| Inline SVG fill attributes for favicon + og-card | sharp/librsvg does not reliably process `<style>` blocks at rasterization | ✓ Good — v1.11 (reused phase 51→52) |
| og-card.jpg filename (was og-image.jpg) | iMessage/Slack/Discord/X/Facebook cache by URL — rename busts the cache | ✓ Good — v1.11 |
| Pipeline fonts in scripts/fonts/ | .astro/fonts/ does not exist at prebuild/pipeline run time | ✓ Good — v1.11 |
| Static public/robots.txt (not dynamic endpoint) | Static site has a fixed URL — no templating benefit, zero-risk | ✓ Good — v1.11 |
